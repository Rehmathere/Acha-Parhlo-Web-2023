...
export default function PAppointments() {
  ...
  const getData = async () => {
    ...
    const sortedVal = dbVal.docs.map((doc) => ({ ...doc.data(), id: doc.id })).sort((a, b) => new Date(a.Date) - new Date(b.Date));
    setVal(sortedVal);
  };
  useEffect(() => {
    getData();
          ...
          <div id="PR_Second">
            <div id="sub_PR_Second">
              <div id="PR_Second_Box">
                <div id="PR_Second_Box_Part_0">Date</div>
                <div id="PR_Second_Box_Part_1">Status</div>
                <div id="PR_Second_Box_Part_2">Name</div>
                <div id="PR_Second_Box_Part_3">Action</div>
              </div>
            </div>
          </div>
          {/* 3 - PR_Third */}
            ...
            <div id="sub_PR_Third">
              ...
              {val.map((values) => (
                <div id="PR_Third_Box">
                  {/* <div id="PR_Third_Box"> */}
                  <div
                    id="PR_Third_Box_Part_0"
                    onClick={() => handleEdit(values.id)}
                  >
                    {values.Date}
                  </div>
                  <div
                    id="PR_Third_Box_Part_1"
                    onClick={() => handleEdit(values.id)}
                  >
                    <img
                      src={getStatusImage(values.status)}
                      alt={values.status}
                    />
                  </div>
                  <div
                    id="PR_Third_Box_Part_2"
                    onClick={() => handleEdit(values.id)}
                  >
                    {values.value_1}
                  </div>
